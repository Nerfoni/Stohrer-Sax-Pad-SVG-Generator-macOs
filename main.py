...
LIGHTBURN_COLORS = [
    ...
    ("06 - Cyan", "#00E0E0"),
    ("07 - Magenta", "#FF00FF"),
    ("08 - Light Gray", "#B4B4B4"),
    ("09 - Dark Blue", "#0000A0"),
    ("10 - Dark Red", "#A00000"),
    ("11 - Dark Green", "#00A000"),
    ("12 - Dark Yellow", "#A0A000"),
    ("13 - Brown", "#C08000"),
    ("14 - Light Blue", "#00A0FF"),
    ("15 - Dark Magenta", "#A000A0"),
    ("16 - Gray", "#808080"),
    ("17 - Periwinkle", "#7D87B9"),
    ("18 - Rose", "#BB7784"),
    ("19 - Cornflower", "#4A6FE3"),
    ("20 - Cerise", "#D33F6A"),
    ("21 - Light Green", "#8CD78C"),
    ("22 - Tan", "#F0B98D"),
    ("23 - Pink", "#F6C4E1"),
    ("24 - Lavender", "#FA9ED4"),
    ("25 - Purple", "#500A78"),
...
RESONANCE_MESSAGES = [
    "Resonance added!",
    "Pad resonance increased!",
    "More resonance now!",
    "Timbral focus enhanced!",
    "Harmonic alignment optimized!",
    "Acoustic reflection matrix calibrated!",
    "Core vibrations synchronized!",
    "Nodal points stabilized!",
    "Overtone series enriched!",
    "Sonic clarity has been improved!",
    "Relacquer devaluation reversed!",
    "Heavy mass screws ain't SHIT!",
    "Now you don't even have to fit the neck!",
    "Let's call this the ULTRAhaul!",
    "Now safe to use hot glue!",
...
class ConfirmationDialog(tk.Toplevel):
    def __init__(self, parent, title, message):
        ...
        checkbox_frame = tk.Frame(self, bg=COLORS["bg_secondary"])
...
class PadSVGGeneratorApp:
    ...
    def set_background_color(self, parent, color):
        ...
        for widget in parent.winfo_children():
            if isinstance(
                widget,
                (tk.Frame, tk.Label, tk.Radiobutton, tk.Checkbutton, tk.LabelFrame),
            ):
    ...
    def create_menu(self):
        ...
        options_menu.add_command(
            label="Sizing Rules...", command=self.open_options_window
        )
        options_menu.add_command(
            label="Layer Colors...", command=self.open_color_window
        )
    ...
    def create_widgets(self):
        tk.Label(
            self.root,
            text="Enter pad sizes (e.g. 42.0x3):",
            bg=self.colors["bg_main"],
            fg=self.colors["text"],
        ).pack(pady=5)
        self.pad_entry = tk.Text(
            self.root, height=10, fg=self.colors["entry_fg"], bg=self.colors["entry_bg"]
        )
        ...
        self.preset_menu.bind(
            "<<ComboboxSelected>>", lambda e: self.on_load_preset(self.preset_var.get())
        )
        ...
        for m in self.material_vars:
            tk.Checkbutton(
                self.root,
                text=m.replace("_", " ").capitalize(),
                variable=self.material_vars[m],
                bg=self.colors["bg_main"],
                fg=self.colors["text"],
        ...
        hole_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=5)
        ...
        tk.Radiobutton(
            ...
            value="No center holes",
            bg=self.colors["bg_main"],
            fg=self.colors["text"],
            command=self.toggle_custom_hole_entry,
        ).pack(side="left")
        tk.Radiobutton(
            hole_frame,
            text="3.0mm",
        ...
        self.custom_hole_entry = tk.Entry(
            hole_frame, width=6, fg=self.colors["entry_fg"], bg=self.colors["entry_bg"]
        )
        self.custom_hole_entry.insert(0, self.settings.get("custom_hole_size", "4.0"))
        self.custom_hole_entry.pack(side="left", padx=2)
        tk.Label(
            hole_frame, text="mm", bg=self.colors["bg_main"], fg=self.colors["text"]
        ).pack(side="left")
        ...
        sheet_frame.grid(row=1, column=0, columnspan=2, sticky="ew", pady=5)

        self.unit_label = tk.Label(
            sheet_frame,
            text=f"Width ({self.settings['units']}):",
            bg=self.colors["bg_main"],
            fg=self.colors["text"],
        )
        self.unit_label.grid(row=0, column=0, sticky="w", padx=5)
        self.width_entry = tk.Entry(
            sheet_frame, fg=self.colors["entry_fg"], bg=self.colors["entry_bg"]
        )
        ...
        self.height_label = tk.Label(
            sheet_frame,
            text=f"Height ({self.settings['units']}):",
            bg=self.colors["bg_main"],
            fg=self.colors["text"],
        )
        ...
        tk.Label(
            self.root,
            text="Output filename base (no extension):",
            bg=self.colors["bg_main"],
            fg=self.colors["text"],
        ).pack(pady=5)
        self.filename_entry = tk.Entry(
            self.root, fg=self.colors["entry_fg"], bg=self.colors["entry_bg"]
        )
    ...
    def open_resonance_window(self):
        ResonanceWindow(
            self.root, self.settings, self.save_settings, self.apply_resonance_theme
        )
    ...
    def get_hole_dia(self):
        ...
        if hole_option == "3.5mm":
            return 3.5
        if hole_option == "3.0mm":
            return 3.0
        if hole_option == "Custom":
            try:
                ...
                messagebox.showerror(
                    "Invalid Input", "Custom hole size must be a valid number."
                )
                return None
        return 0
    ...
    def on_generate(self):
        try:
            ...
            if self.settings.get("engraving_on", True):
                oversized_engravings = check_for_oversized_engravings(
                    pads, self.material_vars, self.settings
                )
                if oversized_engravings and self.settings.get(
                    "show_engraving_warning", True
                ):
                    ...
                    dialog = ConfirmationDialog(
                        self.root, "Engraving Size Warning", message
                    )
            ...
            if self.settings["units"] == "in":
                ...
                messagebox.showerror(
                    "Error", f"Unknown unit '{self.settings['units']}' in settings."
                )
                return
            ...
            for material, var in self.material_vars.items():
                if var.get() and not can_all_pads_fit(
                    pads, material, width_mm, height_mm, self.settings
                ):
                    messagebox.showerror(
                        "Nesting Error",
                        f"Could not fit all '{material.replace('_',' ')}' pieces on the specified sheet size.",
                    )
                    return
            ...
            messagebox.showerror(
                "An Error Occurred", f"Something went wrong during generation:\n\n{e}"
            )
    ...
    def on_save_preset(self):
        ...
        if name:
            ...
            try:
                ...
                messagebox.showinfo(
                    "Preset Saved",
                    f"Preset '{name}' saved successfully to:\n{PRESET_FILE}",
                )
            except PermissionError:
                messagebox.showerror(
                    "Permission Error",
                    f"Cannot save preset to:\n{PRESET_FILE}\n\nPlease check file permissions or run the application with appropriate privileges.",
                )
            except Exception as e:
                messagebox.showerror(
                    "Error Saving Preset",
                    f"Could not save preset to:\n{PRESET_FILE}\n\nError: {e}",
                )
    ...
    def on_delete_preset(self):
        ...
        if selected and selected != "Load Preset":
            if messagebox.askyesno(
                "Delete Preset",
                f"Are you sure you want to delete the preset '{selected}'?",
            ):
                ...
                try:
                    ...
                    self.pad_entry.delete("1.0", tk.END)
                    messagebox.showinfo(
                        "Preset Deleted", f"Preset '{selected}' deleted."
                    )
                except PermissionError:
                    messagebox.showerror(
                        "Permission Error",
                        f"Cannot save preset file to:\n{PRESET_FILE}\n\nPlease check file permissions or run the application with appropriate privileges.",
                    )
                except Exception as e:
                    messagebox.showerror(
                        "Error Deleting Preset",
                        f"Could not save preset file to:\n{PRESET_FILE}\n\nError: {e}",
                    )
...
class OptionsWindow:
    def __init__(self, parent, app, settings, update_callback, save_callback):
        ...
        # --- Main Layout Frames ---
        bottom_button_frame = tk.Frame(self.top, bg=self.colors["bg_secondary"])
        ...
        tk.Button(
            bottom_button_frame,
            text="Advanced",
            command=self.app.open_resonance_window,
            fg=self.colors["button_fg"],
            bg=self.colors["button_bg"],
            relief="flat",
            bd=1,
        ...
        self.canvas = tk.Canvas(
            main_canvas_frame, bg=self.colors["bg_secondary"], highlightthickness=0
        )
        self.scrollbar = tk.Scrollbar(
            main_canvas_frame, orient="vertical", command=self.canvas.yview
        )
        self.scrollable_frame = tk.Frame(
            self.canvas, bg=self.colors["bg_secondary"], padx=10, pady=10
        )
        ...
        self.leather_mult_var = tk.DoubleVar(
            value=self.settings["leather_wrap_multiplier"]
        )
        ...
        self.felt_thickness_unit_var = tk.StringVar(
            value=self.settings["felt_thickness_unit"]
        )
    ...
    def create_option_widgets(self):
        ...
        tk.Label(
            rules_frame,
            text="Felt Diameter Reduction (mm):",
            bg=self.colors["bg_secondary"],
            fg=self.colors["text"],
        ).grid(row=0, column=0, sticky="w", pady=2)
        tk.Entry(
            rules_frame,
            textvariable=self.felt_offset_var,
            width=10,
        ...
        tk.Label(
            rules_frame,
            text="Card Additional Reduction (mm):",
            bg=self.colors["bg_secondary"],
            fg=self.colors["text"],
        ).grid(row=1, column=0, sticky="w", pady=2)
        tk.Entry(
            rules_frame,
            textvariable=self.card_offset_var,
            width=10,
            fg=self.colors["entry_fg"],
            bg=self.colors["entry_bg"],
        ...
        tk.Label(
            rules_frame,
            text="Leather Wrap Multiplier (1.00=default):",
            bg=self.colors["bg_secondary"],
            fg=self.colors["text"],
        ).grid(row=2, column=0, sticky="w", pady=2)
        tk.Entry(
            rules_frame,
            textvariable=self.leather_mult_var,
            width=10,
            fg=self.colors["entry_fg"],
            bg=self.colors["entry_bg"],
        ...
        tk.Label(
            rules_frame,
            text="Min. Pad Size for Hole (mm):",
            bg=self.colors["bg_secondary"],
            fg=self.colors["text"],
        ).grid(row=3, column=0, sticky="w", pady=2)
        tk.Entry(
            rules_frame,
            textvariable=self.min_hole_size_var,
            width=10,
            fg=self.colors["entry_fg"],
            bg=self.colors["entry_bg"],
        ...
        tk.Entry(
            felt_thickness_frame,
            textvariable=self.felt_thickness_var,
            width=10,
            fg=self.colors["entry_fg"],
            bg=self.colors["entry_bg"],
        ).pack(side="left", padx=5)
        tk.Radiobutton(
            felt_thickness_frame,
            text="in",
            variable=self.felt_thickness_unit_var,
            value="in",
            bg=self.colors["bg_secondary"],
            fg=self.colors["text"],
        ).pack(side="left")
        ...
        tk.Checkbutton(
            engraving_frame,
            text="Show Size Label (Engraving)",
            variable=self.engraving_on_var,
            bg=self.colors["bg_secondary"],
            fg=self.colors["text"],
            selectcolor=self.colors["entry_bg"],
        ).pack(anchor="w")

        font_size_frame = tk.LabelFrame(
            engraving_frame,
            text="Font Sizes (mm)",
            bg=self.colors["bg_secondary"],
            fg=self.colors["text"],
        ...
        for i, material in enumerate(materials):
            tk.Label(
                font_size_frame,
                text=f"{material.replace('_', ' ').capitalize()}:",
                bg=self.colors["bg_secondary"],
                fg=self.colors["text"],
            ).grid(row=i, column=0, sticky="w", padx=5, pady=2)
            font_size_var = tk.DoubleVar(
                value=self.settings["engraving_font_size"].get(material, 2.0)
            )
        ...
        engraving_loc_frame = tk.LabelFrame(
            main_frame,
        ...
        for material in materials:
            frame = tk.Frame(engraving_loc_frame, bg=self.colors["bg_secondary"])
            frame.pack(fill="x", pady=2)
            tk.Label(
                frame,
                text=material.replace("_", " ").capitalize() + ":",
                bg=self.colors["bg_secondary"],
                fg=self.colors["text"],
                width=10,
                anchor="w",
            ...
            mode_var = tk.StringVar(
                value=self.settings["engraving_location"][material]["mode"]
            )
            val_var = tk.DoubleVar(
                value=self.settings["engraving_location"][material]["value"]
            )
            ...
            tk.Label(
                frame, text="mm", bg=self.colors["bg_secondary"], fg=self.colors["text"]
            ).pack(side="left")
    ...
    def revert_to_defaults(self):
        if messagebox.askyesno(
            ...
            for material, vars in self.engraving_loc_vars.items():
                vars["mode"].set(
                    DEFAULT_SETTINGS["engraving_location"][material]["mode"]
                )
                vars["value"].set(
                    DEFAULT_SETTINGS["engraving_location"][material]["value"]
                )
...
class LayerColorWindow:
    def __init__(self, parent, settings, save_callback):
        ...
        main_frame = tk.Frame(
            self.top, bg=self.colors["bg_secondary"], padx=10, pady=10
        )
        ...
        layer_map_keys = [
            ...
            "leather_engraving",
            "exact_size_outline",
            "exact_size_center_hole",
            "exact_size_engraving",
        ]
        ...
        for i, key in enumerate(layer_map_keys):
            ...
            combo = ttk.Combobox(
                main_frame, textvariable=var, values=color_names, state="readonly"
            )
...
class ResonanceWindow(tk.Toplevel):
    def __init__(self, parent, settings, save_callback, theme_callback):
        ...
        main_frame = tk.Frame(self, bg=COLORS["bg_secondary"])
        main_frame.pack(expand=True)

        res_button = tk.Button(
            main_frame,
            text="Add Resonance",
    ...
    def start_resonance(self):
        self.withdraw()
        ResonanceProgressDialog(
            self.parent, self.settings, self.save_callback, self.theme_callback
        )
        self.destroy()
...
class ResonanceProgressDialog(tk.Toplevel):
    def __init__(self, parent, settings, save_callback, theme_callback):
        ...
        tk.Label(
            self,
            text="Applying resonance...",
            bg=COLORS["bg_secondary"],
            fg=COLORS["text"],
        ).pack(pady=10)
        self.progress = ttk.Progressbar(
            self, orient="horizontal", length=250, mode="determinate"
        )
    ...
    def finish_resonance(self):
        ...
        if clicks >= 100:
            ...
            UninstallResonanceDialog(
                self.parent_app, self.settings, self.save_callback, self.theme_callback
            )
        else:
            self.save_callback()
...
def generate_svg(
    pads, material, width_mm, height_mm, filename, hole_dia_preset, settings
):
    spacing_mm = 1.0
    discs = []
    ...
    for pad_size, cx, cy, r in placed:
        dwg.add(
            dwg.circle(
                center=(f"{cx}mm", f"{cy}mm"),
                r=f"{r}mm",
                stroke=layer_colors[f"{material}_outline"],
                fill="none",
                stroke_width="0.1mm",
            )
        )
        ...
        if hole_dia > 0:
            dwg.add(
                dwg.circle(
                    center=(f"{cx}mm", f"{cy}mm"),
                    r=f"{hole_dia / 2}mm",
                    stroke=layer_colors[f"{material}_center_hole"],
                    fill="none",
                    stroke_width="0.1mm",
                )
            )
        ...
        if settings.get("engraving_on", True) and (font_size < r * 0.8):
            ...
            if mode == "from_outside":
                ...
                hole_r = (
                    hole_dia / 2 if hole_dia > 0 else 1.75
                )  # Use a small default if no hole for centering
